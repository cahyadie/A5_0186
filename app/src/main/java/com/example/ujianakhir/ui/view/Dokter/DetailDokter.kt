...
fun DetailDokterScreen(
    ...
    val dktruiState by viewModel.detaildktrUiState.collectAsState()
    val scrollBehavior = TopAppBarDefaults.enterAlwaysScrollBehavior()

    LaunchedEffect(Unit) {
        viewModel.getDokter(iddokter)
    }
        ...
        floatingActionButton = {
            Box(
                ...
                Row(
                    ...
                    FloatingActionButton(
                        onClick = {
                            navController.navigate("updateDokter/$iddokter")
                        },
                    ...
                    FloatingActionButton(
                        onClick = {
                            // Action untuk delete
                            viewModel.deleteDokter(iddokter)
                            navController.navigateUp() // Kembali setelah delete
                        },
                        shape = MaterialTheme.shapes.medium,
                        containerColor = Color.Red
                    ) {
                        Icon(imageVector = Icons.Default.Delete, contentDescription = "Delete Dokter")
                    }
                }
            }
        }
...
@Composable
fun DetailBodyDokter(
    detaildktrUiState: DetaildktrUiState,
    modifier: Modifier = Modifier
) {
    when (detaildktrUiState) {
        is DetaildktrUiState.Loading -> {
            CircularProgressIndicator(modifier = modifier.fillMaxSize())
        }
        is DetaildktrUiState.Error -> {
            Text(
                text = detaildktrUiState.message,
                color = Color.Red,
                modifier = modifier.fillMaxSize().wrapContentSize(Alignment.Center)
            )
        }
        is DetaildktrUiState.Success -> {
            val dokter = detaildktrUiState.dokter
            Column(
                verticalArrangement = Arrangement.spacedBy(18.dp),
                modifier = Modifier.padding(12.dp)
            ) {
                ComponentDetailDktr(judul = "Id Dokter", isinya = dokter.iddokter)
                ComponentDetailDktr(judul = "Nama Dokter", isinya = dokter.namadokter)
                ComponentDetailDktr(judul = "Spesialisasi", isinya = dokter.spesialisasi)
                ComponentDetailDktr(judul = "kontak", isinya = dokter.kontak)
